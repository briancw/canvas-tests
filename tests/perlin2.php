...
$perlin = New Perlin();
$height_map = array();
// print_r($perlin->perlinNoise2d(10,10));
$cube_size = 100;
for($x=0; $x<$cube_size; $x++){
	for($y=0; $y<$cube_size; $y++){

		// $tmp_perlin = $perlin->perlinNoise2d($x,$y);
		// $tmp_perlin = $perlin->noise($x,$y);
		$tmp_perlin = $perlin->InterpolatedNoise($x+100,$y+100);
		// $height_map[] = array(
		// 	'x'=>$x,
		// 	'y'=>$y,
		// 	'val'=>$perlin->perlinNoise2d(10,10)
		// );
		echo '<div style="position:absolute; width:3px; height:3px; background-color:rgba(0,0,0,'.$tmp_perlin.'); top:'.($x*3).'; left:'.($y*3).';"></div>';
